...
6. 之后每天自动更新数据（页面运行时直读，无需重新部署），也可手动 **Run workflow** 立即更新
...
`output` 分支的 `stars.json` 由工作流自动生成（GitHub API 拉取 star → 对比上次数据，仅新增项目调用 AI 分类 + 双语简介）。页面运行时直接从 `raw.githubusercontent.com` 读取该文件（跨域已开放），数据更新即时生效；本地开发或远端不可用时回退到内置示例数据。
